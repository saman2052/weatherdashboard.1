...
function searchCity(cityName){
 // This is the build URL to query the database
 console.log(cityName);
 var queryURL = "https://api.openweathermap.org/data/2.5/weather?q=" + 
 cityName + "&appid=" + APIKey;

 // Can run the AJAX call to the OpenWatherAPI
 $.ajax({
   url: queryURL,
   method: "GET"
 })

 // This stores all of the retrieved data inside of an object called "response"
   ...
   .then(function(response) {
     ...
     .then(function(response) {
       ...
       for (var i=0; i < 5; i++) {
        ...
        //  The dateValue = moment().tz(country + "/" + city).add(i, 'days').format('l');
